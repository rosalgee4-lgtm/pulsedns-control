...
```bash
( tmp="$(mktemp)" && trap 'rm -f "$tmp"' EXIT && curl --proto '=https' --proto-redir '=https' --connect-timeout 10 --max-time 120 -fLSs 'https://raw.githubusercontent.com/rosalgee4-lgtm/pulsedns-control/main/public/panel-install.sh?v=0.8.0' -o "$tmp" && test "$(sha256sum "$tmp" | awk '{print $1}')" = '060b83ceb387ee08ad0a1862a75e46ce85120db29102737538b8de0c9e314cd0' && grep -Fq '# PulseDNS Web 主控一键安装与管理脚本' "$tmp" && bash -n "$tmp" && bash "$tmp" install )
```
...
```bash
( tmp="$(mktemp)" && trap 'rm -f "$tmp"' EXIT && curl --proto '=https' --proto-redir '=https' --connect-timeout 10 --max-time 120 -fLSs 'https://raw.githubusercontent.com/rosalgee4-lgtm/pulsedns-control/main/public/panel-install.sh?v=0.8.0' -o "$tmp" && test "$(sha256sum "$tmp" | awk '{print $1}')" = '060b83ceb387ee08ad0a1862a75e46ce85120db29102737538b8de0c9e314cd0' && grep -Fq '# PulseDNS Web 主控一键安装与管理脚本' "$tmp" && bash -n "$tmp" && bash "$tmp" update )
```
...
在 Web 控制台创建节点后只会得到一条节点专属脚本下载直链，不会把整段 Bash 脚本返回到页面。云平台访问它时，主控按当前发布版本动态生成脚本，所以脚本修复后无需重新创建节点；平台可为启动重试重复下载，直到本次预配成功后直链自动失效。下载的脚本会先在最小化系统中补齐 `curl`、CA 证书、`coreutils` 与 `util-linux`，再等待主控，把过程写入 `/var/log/pulsedns-bootstrap.log`、使用由内核在进程结束时自动释放的 `flock` 避免并发，并只在全部步骤成功后写入该节点专属的完成标记；后续重启只启动现有 DDNS 服务，不会覆盖令牌或重复安装 Nyanpass。直接打开交互式操作菜单时，也从固定的 GitHub HTTPS 发布通道下载安装器并校验 SHA-256（目标机需要 `sha256sum`）：

```bash
(
  tmp="$(mktemp)" &&
  trap 'rm -f "$tmp"' EXIT &&
  curl --proto '=https' --proto-redir '=https' -fLSs https://raw.githubusercontent.com/rosalgee4-lgtm/pulsedns-control/release-v0.8.0/public/install.sh -o "$tmp" &&
  test "$(sha256sum "$tmp" | awk '{print $1}')" = 'b0a2acbf9ac54e3c2488563bd74b1a4baddd82584951416029ca3c3d5774e402' &&
...
Web 中“添加探针节点”会先要求填写一次性 root 密码，并预配一个或多个 Nyanpass 服务名、官方命令及原脚本的 `OPTIMIZE` 选项。主控把 root 密码、节点原始令牌和规范化后的预配参数绑定 `nodeId + generation`，以 AES-GCM 加密暂存；页面只返回节点专属下载直链，访问时才解密并按当前代码动态生成脚本。脚本仍执行完整流程：补齐基础环境 → 主控令牌预校验 → SSH → DDNS 安装 → 首次地址上报验收 → 全部 Nyanpass → BBR → DDNS 复检。除安装运行这份脚本所必需的基础环境外，只有主控确认节点令牌有效，并接受至少一次公网地址上报后才会开始修改 SSH、安装 DDNS 与 Nyanpass；重新配置时会清除旧 IP 缓存。执行过程中不再要求粘贴 Nyanpass 命令或确认；每个实例通过原安装器支持的 `S` 与 `OPTIMIZE` 环境变量自动安装。若下载后的脚本超过 15 KiB 安全线，主控会拒绝下发；遇到限制时减少首次预配数量，节点上线后再远程添加其余实例。
...
HTTP 面板上的下载直链复制按钮包含兼容回退，并明确显示“复制成功”或“复制失败”。没有成功提示时不要粘贴，避免使用剪贴板中残留的其他节点旧链接。
...
(
  tmp="$(mktemp)" &&
  trap 'rm -f "$tmp"' EXIT &&
  curl --proto '=https' --proto-redir '=https' -fLSs https://raw.githubusercontent.com/rosalgee4-lgtm/pulsedns-control/release-v0.8.0/public/update.sh -o "$tmp" &&
  test "$(sha256sum "$tmp" | awk '{print $1}')" = '293b73c7bdc5ca8893130ff7f2357796a25b66b06df5c099f29919062fee5fb9' &&
  bash "$tmp"
)
```

升级器支持 HTTPS 地址，以及带端口和 32 位随机路径的 HTTP 主控地址。它先补齐 `curl`、`jq`、`coreutils` 与 `util-linux`，再从 GitHub HTTPS 的 `release-v0.8.0` 发布通道下载专用 `monitor.sh` 并校验代码内固定的 SHA-256；不会从 HTTP 主控执行 root 脚本，也不会下载或执行完整安装器。它只替换 DDNS 探针，不接受新的主控地址或令牌，也不改 systemd 单元、配置、SSH、BBR 或 Nyanpass。配置、IP 缓存和日志不会被清空；运行中的服务会重启并继续正常检测，若公网 IP 此时已经变化，仍会按原逻辑上报并更新 DNS。停止的服务保持停止。新版启动失败时会恢复旧探针，并在 `/opt/ddns-monitor/monitor.sh.previous` 保留上一版本。
