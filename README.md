...
```bash
( tmp="$(mktemp)" && trap 'rm -f "$tmp"' EXIT && curl --proto '=https' --proto-redir '=https' --connect-timeout 10 --max-time 120 -fLSs 'https://raw.githubusercontent.com/rosalgee4-lgtm/pulsedns-control/0fdfb19eb31cb4adb2daba678af8f7b5d00166a1/public/panel-install.sh?v=0.8.2' -o "$tmp" && test "$(sha256sum "$tmp" | awk '{print $1}')" = 'bcf2d90071468b44652c8e989b0d86818bfb88167124c8b3b93bb83c9190a36f' && grep -Fq '# PulseDNS Web 主控一键安装与管理脚本' "$tmp" && bash -n "$tmp" && bash "$tmp" install )
```
